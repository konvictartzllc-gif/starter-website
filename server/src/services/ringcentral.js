...
async function loadSdk() {
    ...
    try {
          ...
          console.warn(`RingCentral SDK unavailable. Calls/SMS disabled. ${err.message}`);
          SDKModule = null;
          return null;
    }
}
...
export async function initRingCentral() {
  ...
  if (!clientId || !clientSecret || !username || !password) {
        console.warn("RingCentral not configured. Calls/SMS disabled.");
        return;
  }
...
export async function sendSms(to, body) {
    if (!rc) return console.warn("RingCentral not ready, SMS skipped.");
    try {
...
export async function makeCall(to, message) {
    if (!rc) return console.warn("RingCentral not ready, call skipped.");
    try {
          const from = process.env.RC_PHONE_NUMBER;
